...
import { Search, LogOut, Settings, ChefHat } from 'lucide-react';

const StaffDashboard = () => {
  const { branchId, signOut } = useAuth();
    ...
    <Card className="shadow-card border-0 mb-3 animate-fade-in">
      <CardContent className="p-4">
        ...
        {order.status === 'pending' && (
          <Button size="sm" className="w-full" onClick={() => updateStatus.mutate({ orderId: order.id, status: 'preparing' })}>
            Mark Preparing
          </Button>
        )}
        {order.status === 'preparing' && (
          <Button size="sm" className="w-full" onClick={() => updateStatus.mutate({ orderId: order.id, status: 'served' })}>
            Mark Served
          </Button>
        )}
      </CardContent>
      ...
      <header className="sticky top-0 z-50 bg-card/95 backdrop-blur-md border-b shadow-card">
        <div className="container mx-auto px-4 flex items-center justify-between h-16">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-lg font-bold">Staff Dashboard</span>
          </div>
